    ...
    if (form[1]["value"]) {
        $.ajax({
            ...
            complete: function(data) {
                $('.houses').append('<div class="row house house-'+data.responseJSON.house_id+'"><!-- Sales Card --><div class="col"><div class="card info-card invite-card"><div class="card-body invite-card-body"><div class="d-flex align-items-center"><div class="card-icon rounded-circle d-flex align-items-center justify-content-center"><i class="bi bi-house"></i></div><div class="ps-3"><h6>'+form[1]["value"]+'</h6></div><div class="delete-regular-day-form"><form action="" name="delete-nh-record-form" class="delete-nh-record-form"><input type="hidden" name="csrfmiddlewaretoken" value="'+csrf+'"><input type="hidden" name="territory" value="'+territory_id+'"><input type="hidden" name="street" value="'+street_id+'"><input type="hidden" name="nh-record" value="'+data.responseJSON.house_id+'"><a class="btn btn-danger btn-sm edit-invite-button delete-nh-record-button"><i class="bi bi-trash"></i></a></form></div></div></div></div></div><!-- End Sales Card --></div>');
                $('.modal').hide();
